...
def main():
    ...
    ckpt = Path("checkpoints/ebm_cifar10.pth")
    if not ckpt.exists():
        raise FileNotFoundError(f"{ckpt} not found. Run train_energy_diffusion.py first.")

    # 1) Load EBM
    ebm = get_model("EBM")
    ebm = load_model(ebm, str(ckpt), device)

    # 2) Generate samples
    n_samples = 25      # 5x5 grid
    steps = 200         # more Langevin steps for better images
    torch.manual_seed(0)

    print(f"Generating {n_samples} EBM samples with {steps} Langevin steps...")
    ...
    out_path = Path("ebm_samples.png")
    nrow = int(math.sqrt(n_samples))
    vutils.save_image(imgs_vis, out_path, nrow=nrow)

    print(f"Saved EBM sample grid to {out_path.resolve()}")
